...
#### **사실 위에서 한 작업들로만으로도 Azure extentions을 이용해 VSCODE로 Cloud shell에 연결해 사용 가능합니다.**  
...
#### **1,2번은 이미 완료했습니다. 이후 local에서 Azure cli에 연결하는 방법은 다음과 같습니다.**  
...
* #### **정상적으로 Cluster를 추가했으나 옆의 목록에 뜨지 않는 이슈가 있네요**  
    ...
    * **해당 이슈는 vscode에서 실행하는 kubectl.exe의 환경 변수 처리가 정상적으로 등록되지 않아서 발생합니다.**  


<br/>

* #### **기본적으로 vscode에서 kubernetes extentions을 설치하면 아래 위치에 tools이 설치됩니다.**
...
* #### **그러나 해당 kubectl.exe를 인식하는 환경변수에 대한 설정이 자동으로 되지 않아 이슈가 발생합니다.**
...
* #### **window 10 -> 제어판 -> 고급 시스템 설정 -> 환경변수에서 ``path``에 해당 경로를 추가해줍니다.**  
...
* #### **해당 작업만 마치게 되면 window cmd , vscode terminer에서 모두 kubectl이 사용 가능합니다.**  
...
### **이제 extention을 사용해서 AKS와 연결하는 방법을 동일하게 진행했지만 아래 에러가 발생합니다.**  
...
### **그래서 vscode kubenetes extentions의 config 설정을 봤더니**
...
### **위 까지 모든 설정을 완료했으면 가이드 대로 AKS를 연결합니다.**  
