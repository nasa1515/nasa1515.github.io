...
* **데이터 유형**
    ...
    |유형|설명|예|
    |:---|:---|:---|
    |비정형 데이터|데이터가 개체로 존재, 구조화 되지 않아 연산이 불가능|문서, 동영상, 이미지 등의 이진 파일|
...
**즉 Storage Account가 Azure Storage 서비스의 최상위 NameSpace이며, 하위의 스토리지를 사용할 수 있게 권한을 부여합니다.**  
...
* **Performance : Perfomance는 생성 후 변경 할 수 없습니다.**  
    ...
    * **FileStorage : 프리미엄 성능 계층에서 사용할 수 있으며 파일 공유 전용 스토리지 계정입니다. (SQL 등)**
    * **BlockblobStorage : 프리미엄 성능 계층에서 사용, 개체 데이터를 저장하는 Blob과 추가 Blob 전용 스토리지 계정**
...
* **1. LRS(Locally Redundant Storage): 물리적으로 동일한 단일 지역 내에 데이터를 세 번 복제하는 방식입니다.**

    **저렴한 방법이지만 해당 데이터센터에 장애가 발생한 경우에는 데이터를 복구할 수 없게 되기 때문에 가용성은 떨어지는 편입니다.**

    ![다운로드](https://user-images.githubusercontent.com/69498804/107183136-8dc69980-6a21-11eb-8ea3-f63e76ad1eee.png)


    **데이터가 세개의 복제본에 완전히 동기화되어야 하기 때문에 쓰기 작업의 경우 세 개의 복제본에 작업이 마쳐야 완료 됩니다.**
...
**단 보조 지역을 선택하는 것은 불가능하고, 주 지역으로 선택한 곳의 리전 쌍(Paired Region)으로 자동 지정됩니다.**  
**주의 해야 할 것은 보조 지역에 저장되어 있는 데이터는 동기화 되어 있찌 않아, 주 지역과 싱크가 정확히 맞지 않을 수 있습니다.**  
...
* **GZRS(Geo Zone Redundant Storage) : ZRS 방식으로 기본 지역 내의 세곡의 가용성 영역에 동기화 해서 복사.**
...
* **위의 Storage Account 별 Redundancy 방식을 정리하면 아래와 같습니다.** 
    ...
    * **기본적으로 GRS,GZRS의 경우 보조 지역의 데이터의 복제 데이터에 대한 읽기/쓰기 접근이 안됩니다.**
