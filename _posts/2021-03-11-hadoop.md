...
**사실 Hadoop하면 그냥 빅데이터를 분산 저장하는 파일시스템이라고 알면 된다고 한다.**  
**정확한 의미는 잘 모르겠지만 가장 주력으로 두었던 HDFS와 MapReduce 방식을 이해하면**  
**어떤 말인지 대충이해는 가고, 나아가서는 EcoSystem에 대해 이해를 한다면 어떤 느낌인지 감이 올 것이다.**  